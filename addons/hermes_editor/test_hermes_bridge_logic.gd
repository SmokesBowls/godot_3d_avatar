...
func _check_coordination_lane() -> void:
	...
	_assert(
		...
		"status reflects edit_result.success as the 'applied' fact, not prose: got %s" % [report_payload.get("status")]
	)
	_assert(
	...
	_assert(
		typeof(report_payload.get("execution_summary")) == TYPE_STRING and not String(report_payload.get("execution_summary", "")).is_empty(),
		"execution_summary is a real, non-empty fact string, not left for Dragon to derive from body"
	)
	_assert(
		typeof(report_payload.get("message_id")) == TYPE_STRING and not String(report_payload.get("message_id", "")).is_empty(),
